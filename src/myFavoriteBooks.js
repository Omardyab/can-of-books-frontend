...
class MyFavoriteBooks extends React.Component {
  ...
  getBooks =async()=>{
    ...
    let booksURL = `http://localhost:3010/book?email=${user.email}`;
  ...
  addBook=async (event)=>{
    ...
    const bookData = {
      ...
      state: event.target.status.value,
    ...
    // console.log(bookData);
      ...
      //  let newBook =await axios.post(`${process.env.REACT_APP_BOOKS}/addBook`, {params: bookData});
       let newBook =await axios.post(`http://localhost:3010/addBook`, bookData);
  ...
  deleteBook=( index)=>{
    ...
    axios
        .delete(`http://localhost:3010/deleteBook/${index}`, {params: user.email})
        .then((book)=>{ 
          this.setState({
